                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($orders as $order)
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm font-semibold text-gray-800">PHP {{ number_format(auth()->user()->isFarmer() ? $farmerSubtotal : $order->total, 2) }}</td>
                                        ...
                                        @if($isBuyerOrders)
                                            @if($order->canBeCancelledByConsumer())
                                                <form method="POST" action="{{ route('consumer.orders.cancel', $order) }}" onsubmit="return confirm('Are you sure you want to cancel this order?')">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="rounded-lg bg-red-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-red-700">
                                                        Cancel Order
                                                    </button>
                                                </form>
                                            @elseif($order->status !== 'completed')
                                                <span class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-500">{{ $order->consumerCancellationMessage() }}</span>
                                            @endif
                                        ...
                                        @elseif(auth()->user()->isFarmer())
                                            @if($order->status === 'pending')
                                                <form method="POST" action="{{ route('farmer.orders.accept', $order) }}">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="rounded-lg bg-emerald-700 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-800">Accept Order</button>
                                                </form>
                                                <form method="POST" action="{{ route('farmer.orders.cancel', $order) }}" onsubmit="return confirm('Are you sure you want to cancel this order?')">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="rounded-lg bg-red-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-red-700">Cancel Order</button>
                                                </form>
                                            @elseif($order->status === 'accepted')
                                                <form method="POST" action="{{ route('farmer.orders.preparing', $order) }}">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-indigo-700">Mark as Preparing</button>
                                                </form>
                                                <form method="POST" action="{{ route('farmer.orders.cancel', $order) }}" onsubmit="return confirm('Are you sure you want to cancel this order?')">
                                                    @csrf
                                                    @method('PATCH')
                                            ...
                                            @elseif($order->status === 'preparing')
                                                <form method="POST" action="{{ route('farmer.orders.complete', $order) }}">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="rounded-lg bg-emerald-700 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-800">Mark as Completed</button>
                                                </form>
                                            @elseif($order->status === 'completed')
                                                <span class="rounded-lg bg-emerald-100 px-3 py-1.5 text-xs font-bold text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200">Completed</span>
                                            @elseif($order->status === 'cancelled')
                                                <span class="rounded-lg bg-red-100 px-3 py-1.5 text-xs font-bold text-red-800 dark:bg-red-900/40 dark:text-red-200">Cancelled</span>
                                            @endif
                                        @endif
                                    </div>
                                </td>
