...
<div {{ $attributes->merge(['class' => 'rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 px-6 py-12 text-center']) }}>
    ...
    <p class="mx-auto mt-2 max-w-md text-sm leading-relaxed text-slate-500">{{ $message }}</p>

    @if($actionUrl && $actionLabel)
        <a href="{{ $actionUrl }}" class="mt-5 inline-flex items-center justify-center rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-800">
            {{ $actionLabel }}
        </a>
    @endif
</div>
