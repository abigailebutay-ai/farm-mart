...
@section('content')
    ...
    <div class="grid gap-4 md:grid-cols-3">
        ...
        <x-ui.stat-card label="Total Kg Sold" value="{{ ($totalSoldQuantity ?? 0) . ' kg' }}" icon="products" tone="blue" trend="Total kg sold in completed orders." />
    </div>
        ...
        <x-ui.table-card title="Monthly Sales Summary" subtitle="Completed sales grouped by month.">
            <thead class="bg-slate-50">
                <tr class="text-left text-xs font-bold uppercase tracking-wide text-slate-500">
                    ...
                    <th class="px-5 py-3">Quantity Sold (kg)</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($monthlyRows ?? [] as $row)
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-5 py-4 text-sm text-slate-600">{{ $row['quantity'] }} kg</td>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($recentSales ?? [] as $item)
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-5 py-4 text-sm text-slate-600">{{ $item->quantity }} kg</td>
