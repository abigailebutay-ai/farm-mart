...
@if(auth()->check() && request()->routeIs('dashboard', 'orders.*', 'checkout.*', 'cart.*', 'products.show', 'profile.*', 'settings.*', 'consumer.*', 'farmer.*', 'admin.*'))
    ...
    @php
        ...
        if ($user->isAdmin()) {
            ...
            $navItems = [
                ['label' => 'Dashboard', 'href' => route('dashboard'), 'active' => 'dashboard', 'icon' => 'dashboard'],
                ['label' => 'Marketplace', 'href' => route('consumer.marketplace'), 'active' => ['consumer.marketplace', 'products.show'], 'icon' => 'products'],
